...
pub fn decode_frame(b: &[u8]) -> Result<Ws90, FrameError> {
    ...
    Ok(Ws90 {
        ...
        temperature_c: (temp_raw != 0x3ff).then_some((temp_raw as f32 - 400.0) * 0.1),
        humidity_pct: (b[9] != 0xff).then_some(b[9]),
        pressure_hpa: (pressure != 0x3fff).then_some(pressure as f32),
        wind_dir_deg: (wind_dir != 0x1ff).then_some(wind_dir),
        wind_avg_m_s: (wind_avg != 0x1ff).then_some(wind_avg as f32 * 0.1),
        wind_max_m_s: (wind_max != 0x1ff).then_some(wind_max as f32 * 0.1),
        uv_index: (b[13] != 0xff).then(|| b[13] as f32 * 0.1),
        light_lux: (light_raw != 0xffff).then_some(light_raw as f32 * 10.0),
        rain_total_mm: u16::from_be_bytes([b[19], b[20]]) as f32 * 0.1,
        rain_start: b[16] & 0x10 != 0,
        supercap_v: (supercap != 0x3f).then_some(supercap as f32 * 0.1),
        firmware: b[29],
        flags,
    })
}
