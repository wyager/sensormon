        ...
        Cmd::DecodeFile { file, rate, center, format, receiver, summary, block, trace, from, to, chirps_db } => {
            ...
            let mut store = match &chirps_db {
                Some(path) => {
                    ...
                    Some(chirps::ChirpStore::open(chirps::ChirpStoreConfig { path: path.display().to_string(), max_bytes: 50_000_000, max_examples_per_group: 6, receivers: Vec::new(), max_examples_per_family: 50, min_family_interval_s: 0.0 })?)
                }
                None => None,
            };
